...
# -*- coding: utf-8 -*-
"""
DiffusionEdge + STARE Test Script (First Stage Checkpoint Olmadan)
STARE dataset üzerinde DiffusionEdge modelini test eder ve metrikleri hesaplar
    ...
    if 'ema' in checkpoint:
        ...
        for k in sd.keys():
            if k.startswith("ema_model."):
                new_k = k[10:]  # ema_model. prefix'ini kaldır
                new_sd[new_k] = sd[k]
            else:
                new_sd[k] = sd[k]
...
def test_stare_diffusion_edge(model, stare_root, output_dir='stare_diffusion_results', 
                               device='cuda', target_size=(320, 320)):
    """STARE dataset üzerinde DiffusionEdge testini çalıştır"""
    ...
    print("\n" + "="*50)
    print("DiffusionEdge ile STARE testi başlatılıyor...")
    print("="*50)
    ...
    for img_path, gt_path in tqdm(pairs, desc="Test ediliyor"):
        try:
            ...
            output_name = os.path.basename(img_path).replace('.ppm', '_diffusion_pred.png')
    ...
    if len(mse_values) > 0:
        ...
        json_path = os.path.join(output_dir, 'stare_diffusion_results.json')
        with open(json_path, 'w') as f:
            json.dump(results, f, indent=4)
        
        # Terminal çıktısı
        print("\n" + "="*50)
        print("STARE DIFFUSIONEDGE TEST SONUÇLARI (ORTALAMA)")
        print("="*50)
        ...
        print("\nHATA: Hiçbir görüntü işlenemedi!")
        return None
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='DiffusionEdge STARE Test')
    parser.add_argument('--stare_root', type=str, 
                        default=r"C:\Users\emredogan\Desktop\itu_master\fuzzlogicproject\stare",
                        help='STARE dataset root dizini')
    ...
    parser.add_argument('--output_dir', type=str,
                        default='stare_diffusion_results',
    ...
    print(f"\n{'='*60}")
    print(f"DiffusionEdge STARE Test")
    print(f"{'='*60}")
    ...
    # Dataset kontrolü
    if not os.path.exists(args.stare_root):
        print(f"\nHATA: STARE dataset bulunamadı: {args.stare_root}")
        exit(1)
    ...
    try:
        ...
        # Test çalıştır
        results = test_stare_diffusion_edge(
            model=model,
